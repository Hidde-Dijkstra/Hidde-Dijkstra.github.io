...
class _DrawLattice:
    def __init__(self, lattice, W, **kwargs):
        self.W = W
        self.H = kwargs.get("H", W)
        self.D = kwargs.get("D", W)
    ...
    def plot(
        ...
        show_z=False,
        cell_border=True,
        cell_attributes=dict(line=dict(color="black", dash="dash"), name="cell border"),
        **kwargs
    ):
        general_dic = dict(mode="markers", showlegend=False, line_width=1)
        scale = 1 if units == "" else self.a
        general_dic.update(kwargs.get("all", {}))
        fig = go.Figure()
        ...
        if plot_3d:
            ...
            scene = dict(
                xaxis=xaxis, yaxis=yaxis, zaxis=zaxis, aspectratio=dict(x=1, y=1, z=1)
            )
