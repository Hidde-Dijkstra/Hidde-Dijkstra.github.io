...
class Lattice:
    ...
    b_1 = np.array([1, 1 / np.sqrt(3), 0])
    b_2 = np.array([0, 2 / np.sqrt(3), 0])

    def __init__(self, a):
        self.a = a
        self.a_vec = [self.a_1 * a, self.a_2 * a]
        self.b_vec = [2 * np.pi * b / a for b in [self.b_1, self.b_2]]
        self.unit_cell = []
...
class Supercell(Lattice):
    def __init__(self, lattice, m, n):
        ...
        r = max(m, n)
        vec_range = [
            (self.a_1 * i + self.a_2 * j)
            for i in range(-2 * r, 2 * r)
            for j in range(-2 * r, 2 * r)
        ]
        for atom in lattice.unit_cell:
            for vec in vec_range:
                ...
                if self._in_supercell(atom_position / scaling):
                    self._add_atom(
                        atom.copy(atom_position, scaling=scaling), scale=False
                    )

    def _rot_mat(self, θ):
        return np.array(
            [[np.cos(θ), -np.sin(θ), 0], [np.sin(θ), np.cos(θ), 0], [0, 0, 1]]
        )
